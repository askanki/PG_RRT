...
class Tree:
    def __init__(self, canvas, threshold_theta=90, resolution_angle=90, step_size=0.4):
        """
    ...
    def setup_action(self, node, resolution_angle):
        ...
        for angle in range(0, 360, resolution_angle):
            ...
            closest_action = self.canvas.start
            for node_ in self.actions_taken:  # Heavy operation, check if it can be replaced with quad-tree
                if eul_dist(node_, sample_location) < eul_dist(closest_action, sample_location):
                    closest_action = node_

            if eul_dist(closest_action, sample_location) < min(math.sqrt(
                    2 * (self.step_size ** 2) - 2 * (self.step_size ** 2) * math.cos(
                            resolution_angle * math.pi / 180)) - 0.1, self.step_size - 0.05):
                    closest_node = self.canvas.start
                    for node_ in self.nodes:
                        if eul_dist(node_, sample_location) < eul_dist(closest_node, sample_location):
                            closest_node = node_
                    if eul_dist(closest_node, sample_location) < min(math.sqrt(
                    2 * (self.step_size ** 2) - 2 * (self.step_size ** 2) * math.cos(
                            resolution_angle * math.pi / 180)) - 0.1, self.step_size - 0.05):
                        self.add_yaw(node, closest_node)
                    else:
                        closest_pseudo = self.canvas.start
                        for node_ in self.pseudo_nodes_action:
                            if eul_dist(node_, sample_location) < eul_dist(closest_node, sample_location):
                                closest_pseudo = node_
                        self.add_yaw(node, closest_pseudo)

            else:
                self.actions_taken[sample_location] = True
                action.append((angle, sample_location))
                self.pseudo_nodes_action[sample_location] = []
                self.add_yaw(node, sample_location)
    ...
    def add_yaw(self, parent, node):
        ...
        if len(action) > 0:
            if node in self.actions:
                self.actions[node].append(action)
            else:
                self.actions[node] = action
    ...
    while True:
        if len(tree.special_nodes) == 0:
            ...
            for nodes in tree.prev_special_nodes:  # can be optimized
                # plt.plot(nodes[0], nodes[1], color="green", marker="o")
                while nodes in tree.used_special_nodes:
                    # plt.pause(0.01)
                    # plt.plot(nodes[0], nodes[1], color="red", marker="o")
        ...
        if new_node:
            ...
            # plt.plot(parent[0], parent[1], color="red", marker="o")
...
def build_rrt():
    ...
    :return:
    """
    canvas = Canvas(Polygon([(-20, -20), (40, -20), (40, 20), (-20, 20)]), (0.4, 1), (13, -5))
    canvas1 = Canvas(Polygon([(-20, -20), (40, -20), (40, 20), (-20, 20)]), (13, -5), (0.4, 1))
