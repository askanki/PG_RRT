...
class Tree:
    def __init__(self, canvas, threshold_theta=90, resolution_angle=90, step_size=0.25):
        """
        ...
        self.quadtree = Quad(Point(-25, -25), Point(25, 25), 0.1)
        """////////////////////////"""
    ...
    def setup_action(self, node, resolution_angle):
        ...
        for angle in range(0, 360, resolution_angle):
            ...
            flag = 0
            plt.plot(sample_location[0], sample_location[1], color="black", marker=".")
            if self.quadtree.search(Point(sample_location[0], sample_location[1]), 0.2, [Point(x[0], x[1]) for x in self.actions_taken]):
                flag = 1
                #print("break")
            if flag == 0:
                self.actions_taken[sample_location] = True
                action.append((angle, sample_location))
               # print("Node to insert: ", sample_location)
                self.quadtree.insert(Node(Point(sample_location[0], sample_location[1]), (sample_location[0], sample_location[1])))
            plt.plot(sample_location[0], sample_location[1], color="white", marker=".")
        if len(action) > 0:
            self.actions[node] = action
        plt.plot(node[0], node[1], color="grey", marker="o")
