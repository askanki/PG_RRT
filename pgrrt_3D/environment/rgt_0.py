...
class Tree:
    def __init__(self, canvas, threshold_theta=40, resolution_angle=90, step_size=0.5):
        """
        ...
        self.prob_dist[canvas.start] = GMM([Gaussian(0, 10, 0.25), Gaussian(-5 % 360, 10, 0.25), Gaussian(25, 10, 0.1), Gaussian(-25 % 360, 10, 0.1), Gaussian(45, 10, 0.75), Gaussian(-45 % 360, 10, 0.075), Gaussian(60, 10, 0.05), Gaussian(-60 % 360, 10, 0.05), Gaussian(90, 10, 0.025), Gaussian(-90 % 360, 10, 0.025)])
    ...
    def setup_action(self, node, resolution_angle):
        ...
        for angle in range(0, 360, resolution_angle):
            ...
            #     if eul_dist(node_, sample_location) < min(max(math.sqrt(2*(self.step_size**2) - 2*(self.step_size**2)*math.cos(resolution_angle*math.pi/180)) - 0.05, (self.step_size/2)*math.sqrt(2)), self.step_size - .08):
            #         flag = 1
            #         break
            if flag == 0:
    ...
    def add_node(self, parent, node, gauss):
        ...
        for old_node in self.nodes:
            if eul_dist(old_node, node) < self.step_size*0.9:#min(max(math.sqrt(2*(self.step_size**2) - 2*(self.step_size**2)*math.cos(self.resolution_angle*math.pi/180)) - 0.05, (self.step_size/2)*math.sqrt(2)), self.step_size - .08):
                collision_free = False

        if not self.canvas.adv_check_collision(node, parent, self.step_size) and collision_free:   # True if no obstacles
    ...
    def check_threshold(self, node, parent):
        ...
        if min(get_angle(node, parent, self.parent[parent]), abs(360 - get_angle(node, parent, self.parent[parent]))) > 180 - self.threshold_theta:
            return True
        return False
    ...
    def make_action(self, node, sampled_direction):
        ...
        if node in self.actions:
            ...
            for action in actions:
                if dist_mean(action[0], sampled_direction) < dist_mean(closest[0], sampled_direction):  # replace with dist function
                    closest = action
...
def termination(tree1, tree2, new_node):
    ...
    for x in range(len(tree2.nodes)):
        if eul_dist(new_node, tree2.nodes[x]) < 0.5 * tree1.step_size :#and (not tree1.canvas.check_collision(new_node, tree2.nodes[x])):
            return x
    return False
